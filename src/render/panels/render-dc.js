...
import { getOutline } from '../outlines/index.js';
    ...
    // ── Hull outline: custom per vessel or generic pill fallback ─────────────
    const vesselKey = session.vesselKey || '688';
    const customOutline = getOutline(vesselKey);
    let hullPath, phTop, phBot, phMid, phR, phX0, phX1, compXs, compWs;

    if (customOutline) {
      // Vessel-specific outline (draws external equipment + hull + WTD dividers)
      const geo = customOutline(ctx, U, { schX, schW, schY, schH }, dmg,
        { compKeys, compFracs, compLabels, vesselKey });
      hullPath = geo.hullPath;
      phTop = geo.phTop; phBot = geo.phBot; phMid = geo.phMid; phR = geo.phR;
      phX0 = geo.phX0; phX1 = geo.phX1;
      compXs = geo.compXs; compWs = geo.compWs;
    } else {
      // Generic pill fallback (legacy vessels)
      phTop = schY + U(44);
      phBot = schY + U(114);
      phMid = (phTop+phBot)*0.5;
      phR   = (phBot-phTop)*0.5;
      phX0  = schX + phR + U(18);
      phX1  = schX + schW - phR - U(18);
      const phSpan = phX1 - phX0;

      compXs=[]; compWs=[];
      { let xx=phX0;
        for(let i=0;i<compKeys.length;i++){ compWs[i]=phSpan*compFracs[i]; compXs[i]=xx; xx+=compWs[i]; } }
      ...
      const phSpan2 = phX1 - phX0;
      const sailCX  = phX0 + phSpan2*(0.21 + 0.105);
      const sailW   = U(44);
      const sailH   = U(22);
      const sailBot = ohTop;
      ...
      // WTD dividers (legacy — dashed lines between compartments)
      ctx.save(); hullPath(); ctx.clip();
      for(let ci=1;ci<compKeys.length;ci++){
        const x=compXs[ci];
        ctx.strokeStyle='rgba(60,90,140,0.40)';
    ...
    // Alias for compartment fill clipping (used by both paths)
    const pillPath = hullPath;

    // ── Fill compartments inside pill ─────────────────────────────────────────
    for(let ci=0;ci<compKeys.length;ci++){
      ...
      const fw = (ci===0||ci===compKeys.length-1) ? cw+phR : cw;

      ctx.fillStyle=isFlooded?'rgba(8,18,70,0.98)':stFill.nominal;
    ...
    for(let ci=0;ci<compKeys.length;ci++){
      ...
      // Section-wide worst-state glow removed — individual system labels show damage state
      ...
      const fireLevel=Math.max(...(DMG.SECTION_ROOMS[comp]||[]).map(rid=>dmg._fireDetected?.[rid]?(dmg.fire?.[rid]||0):0));
      const hasAlarmDC=(DMG.SECTION_ROOMS[comp]||[]).some(rid=>(dmg._fireAlarmFired?.[rid]||dmg._fireAlarmFalse?.[rid])&&!dmg._fireDetected?.[rid]);
      const drench=dmg._fireDrench?.[comp];
      const drenchLevel=drench?.level??0;
      if(fireLevel>0.02&&drenchLevel<1){
        ctx.save(); pillPath(); ctx.clip();
        const fx2=ci===0?cx2-phR:cx2, fw2=(ci===0||ci===compKeys.length-1)?cw+phR:cw;
      ...
      if(isFlooded){
        ...
        const pulse3=0.5+0.5*Math.sin(performance.now()*0.006);
        ctx.fillStyle=`rgba(255,180,0,${0.6+pulse3*0.3})`;
        ctx.font=`bold ${U(9)}px ui-monospace,monospace`;
        ctx.fillText('ALARM', cMid, phMid+U(4));
      } else if(flood>0.02){
    ...
    // Systems are now rendered inside room blocks in the damage screen (H key).
    ...
    if(session.debugOverlay){
      ...
      const dbgW=(OW-P*2-U(4))/compKeys.length;
